...
const Contacts = () => {
  ...
  const handleOnContactAdd = (payload) => {
    if (payload.contactEmail === "") toast.error("Please Add Email!");
  ...
  useEffect(() => {
    ...
    if (success) {
      if (data?.isContactFeatched) {
        setContacts(data?.contacts);
      }

      if (data?.isContactCreated) {
        const updateContact = [...Contacts, data.contact];
        updateContact.sort((a, b) =>
  ...
  useEffect(() => {
    dispatch(searchContact(searchQ));
  }, [searchQ]);
      ...
      <div>
        <div className="relative box-border">
          ...
          <span className="mt-2 font-normal text-[#6a6d70] block text-[.875em] box-border">
            ...
            <button
              ...
              className=" font-normal text-[1em] text-[#484a4d] cursor-pointer underline bg-none border-none p-0 box-border"
            >
              Add contact
            </button>
          </span>
          <div className=" inline-block m-0 align-top w-[18.75em] absolute top-0 right-0 font-normal bg-[#fff] border border-[#babcbf] rounded-[5px] clear-both text-[#6a6d40] h-[50px]">
            <input
              type="text"
              placeholder="search for email, firstName or lastName"
            ...
            <svg
              className="h-[.875em] w-[.875em] left-[1em] absolute top-[1.125em] box-border font-normal text-[#6a6d70]"
              viewBox="19 18 14 14"
            >
              <path
              ...
              <div className="flex box-border">
                <div className="rounded-[3px] inline-block h-[2.25em] mr-[.625em] mt-0 w-[calc((100%-54px)/3-10px)] font-normal bg-[#fff] border border-[#babcbf] clear-both relative text-[#6a6d70]">
                  <input
                    ...
                    className="rounded-[3px] pl-[.7142857143em] pr-[1.7857142857em] pt-0 font-normal bg-[rgba(0,0,0,0)] border-0 text-[.875em] h-full left-0 pb-0 absolute text-ellipsis w-full z-[2] box-border  outline-none focus:shadow-[0_0_0_0.0625em_#5268ff] lowercase"
                  />
                </div>
                ...
                <div className="flex items-center justify-between flex-grow box-border max-w-[54.04px]">
                  {loading ? (
                    ...
                    <svg
                      onClick={() => handleOnContactAdd(payload)}
        ...
        <ul className=" list-none mt-[2.5em] mx-0 mb-[1.875em] box-border">
          {Contacts?.map((contact, key) => {
            const currentEmail = contact.contactEmail[0];
            const previousEmail =
              key > 0 ? Contacts[key - 1].contactEmail[0] : null;
            const showCharLabel = currentEmail !== previousEmail;
